...
newShaderEffect("everythingUP_ShockWave_Med_WARP", 15, "w");
newShaderEffect("everythingUP_ShockWave_Med_COLOR", 15, "c");
...
function AppendAll()
	...
	postfx.append(
		[[
		vec2 everythingUP_ShockWave_Med_WARP(float x, float y, float iTime, vec2 uv){
			...
			float uppercircbound = iTime*1.0;
			float lowercircbound = (uppercircbound + iTime)/1.5;
				
			return uv + (normalize(uvTemp - position) * smoothstep(0.5, 0.0, dist) * (smoothstep(lowercircbound-0.2, lowercircbound, dist) - smoothstep(uppercircbound, uppercircbound, dist)));
		}

		vec2 everythingUP_Magnet(float x, float y, float iTime, vec2 uv){
			// Normalized pixel coordinates (from 0 to 1)
	...
	postfx.append(
	[[
	vec3 everythingUP_ShockWave_Med_COLOR(float x, float y, float iTime, vec2 uv){
		// Normalized pixel coordinates (from 0 to 1)
		...
		return 0.75 * vec3(smoothstep(0.5, 0.0, dist) * (smoothstep(lowercircbound-0.5, lowercircbound, dist) - smoothstep(uppercircbound, uppercircbound, dist)));
	}
	]],
		"// utilities",
		"data/shaders/post_final.frag"
...
ModTextFileSetContent("data/translations/common.csv", ModTextFileGetContent("data/translations/common.csv") .. [[
action_reverbation,Reverbation,,,,,,,,,,,,,
