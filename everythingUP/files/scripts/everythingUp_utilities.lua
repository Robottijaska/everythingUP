...
function getShaderEffects()
    ...
    --hell
    for number, string in input:gmatch("{%s*([^,]+)%s*,%s*([^}]+)%s*}") do
        table.insert(result, {id = tonumber(number), name = string})
    end
...
function setShaderEffects(input)
    ...
    for _, entry in ipairs(input) do
        ...
        table.insert(result, "{" .. number .. ", " .. string .. "}")
    end
...
function insertShaderEffect(id, name)
    local shaders = getShaderEffects()

    print("inserting {" .. id .. ", " .. name .. "}")
    table.insert(shaders, 1, {id = id, name = name})
...
function loadShaderEffect(index, id)
    local shaders = getShaderEffects()
    shaders[index].id = id
...
local function makeShaderEntry(id, name)
    local shaderEffects = getShaderEffects()

    insertShaderEffect(id, name)
...
function newShaderEffect(name, max_num, shadersetting)
	...
	for i = 1, max_num do makeShaderEntry(0, name .. i) end
	for i = 1, max_num do
		...
		if shadersetting == "w" then
		    ...
		    [[
		        if (]].. currentUniform ..[[.w > 0.0) {
		    	    tex_coord.xy = ]] .. name .. [[(]].. currentUniform ..[[.x, ]].. currentUniform ..[[.y, time - ]].. currentUniform ..[[.z, tex_coord.xy);
	    	    }
		    ]],
		    ...
		    [[
		        if (]].. currentUniform ..[[.w > 0.0) {
		    	    color += ]] .. name .. [[(]].. currentUniform ..[[.x, ]].. currentUniform ..[[.y, time - ]].. currentUniform ..[[.z, tex_coord.xy);
	    	    }
		    ]],
...
function spawnShader(lifetime, baseName, x, y, id)
    local shaderEffects = getShaderEffects()

	local index = findFreeShaderLocation(baseName)
	if index > 0 then
        local oldTime = GameGetRealWorldTimeSinceStarted()
        local currentTime = GameGetFrameNum()/60
        ...
        local comp = EntityGetFirstComponentIncludingDisabled(entity_id, "VariableStorageComponent", "shaderProjectile_time")
        comp = ComponentSetValue2( comp, "value_int", currentTime)
        ...
        --GamePrint(shader_x .. ", " .. shader_y)
        newTime = GameGetRealWorldTimeSinceStarted()
        --GamePrint("time diff: " .. newTime / oldTime)

        GameSetPostFxParameter( findNameOfShaderLocation(index, baseName), shader_x, shader_y, currentTime + 60 * (1 - newTime / oldTime), id )

        loadShaderEffect(index, id)
        --print("new shader loaded with id " .. id)
		return id
	end
	return 0
end
